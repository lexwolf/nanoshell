...
if $use_compile; then
  ...
  if [ "$plot_mode" == "num" ]; then
    g++ -Wall -I/usr/local/include -L/usr/local/lib ../src/nanoshell_num.cxx -o ../bin/nsn -lgsl -lgslcblas -lm -larmadillo
  elif [ "$plot_mode" == "anl" ]; then
    g++ -Wall -I/usr/local/include -L/usr/local/lib ../src/nanoshell_anl.cxx -o ../bin/nsa -lgsl -lgslcblas -lm -larmadillo
  else
...
read r1 Dome ome_0 G omemi omema mtl mdl active sol E0 rap hst< $infile
...
do
  ...
  {
      echo "$r1 $Dome $om0 $wG $omemi $omema $mtl $mdl $active $sol $E0 $rap $hst"
  ...
  mkdir -p "../img/output/${file[$j]}/pdf" "../img/output/${file[$j]}/png"
  ...
  do
    ...
    name=$(printf $frm $i)
    mv ../img/output/intime.pdf ../img/output/${file[$j]}/pdf/im-${name}.pdf
    mv ../img/output/intime.png ../img/output/${file[$j]}/png/im-${name}.png
  done
  echo "> Creating ${file[$j]}.gif in ../img/output/GIF/intime ..."
  convert -dispose previous -delay 20 ../img/output/${file[$j]}/png/*.png -loop 0 ../img/output/${file[$j]}.gif
  mv ../img/output/${file[$j]}.gif ../img/output/GIF/intime
  echo "> done!"
done
