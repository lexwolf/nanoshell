...
CXXFLAGS="${CXXFLAGS:-} -I$ROOT_DIR/extern/nano_geo_matrix/include"
...
# Format expected:
# a Dome ome_g G omemi omema mtl mdl active sol E0 rho hst
read -r a Dome ome_g G omemi omema mtl mdl active sol E0 rho hst < "$INPUT_FILE"
...
write_input() {
  ...
  local omema_val="2.9"
    {
    echo "$a $Dome $ome_g_val $G_val $omemi_val $omema_val $mtl $mdl $active $sol $E0 $rho $hst"
...
# Restore "baseline" input first (ome_g, G as in file)
write_input "$ome_g" "$G"
...
echo "> Running steady_state for G = 0.2 * Gth = $new_G (ome_g=$ome_th)"
"$BIN_DIR/sts"
